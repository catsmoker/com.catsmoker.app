...
class NonRootActivity : AppCompatActivity(), Shizuku.OnRequestPermissionResultListener {
    ...
    private suspend fun uploadCustomFileWithShizuku(fileUri: Uri, packageName: String): Boolean {
        ...
        val shizukuTempDir = externalCacheDir ?: cacheDir
        val tempFile = File(shizukuTempDir, "Custom_Active.sav")
    ...
    private suspend fun uploadCustomFolderWithShizuku(folderUri: Uri, packageName: String): Boolean {
        ...
        val shizukuTempDir = externalCacheDir ?: cacheDir
        val tempDir = File(shizukuTempDir, "custom_folder")
    ...
    private fun performShizukuCopy(config: GameConfig) {
        ...
        launchIoWithLoading(
            ...
            try {
                val shizukuTempDir = externalCacheDir ?: cacheDir
                tempFile = File(shizukuTempDir, config.saveFile)
                tempFile.parentFile?.mkdirs()

                val destDir = Environment.getExternalStorageDirectory().path + config.saveDir
                val destPath = destDir + config.saveFile

                // 1. Try to copy existing Active.sav from device save directory to tempFile
                val copyFromGameExit = execShizukuCommands(
                    arrayOf("sh", "-c", "cp -f \"$destPath\" \"${tempFile.absolutePath}\"")
                )

                val inputBytes: ByteArray = if (copyFromGameExit == 0 && tempFile.exists() && tempFile.length() > 0) {
                    tempFile.readBytes()
                } else {
                    assets.open(assetPath).use { it.readBytes() }
                }

                // 2. Perform dynamic binary modification if applying Max FPS
                val finalBytes = if (binding.profileToggleGroup.checkedButtonId == R.id.btn_apply_max_fps) {
                    ActiveSavModifier.modifyFps(inputBytes, targetFps = 8)
                } else {
                    inputBytes
                }
                ...
                // 3. Execute shell commands via Shizuku to copy back modified Active.sav
                ...
                withContext(Dispatchers.Main) {
                    if (exitCp == 0) showSnackbar("Success! Active.sav modified in-place & updated via Shizuku.")
    ...
    private fun performSafFileCopy(treeUri: Uri, game: GameType?) {
        ...
        launchIoWithLoading(
            ...
            val inputBytes: ByteArray = if (existingFile != null && existingFile.length() > 0) {
                contentResolver.openInputStream(existingFile.uri)?.use { it.readBytes() }
                    ?: assets.open(assetPath).use { it.readBytes() }
            } else {
                assets.open(assetPath).use { it.readBytes() }
            }
            ...
            val targetFile = existingFile ?: pickedDir.createFile(MIME_BINARY, config.saveFile)
