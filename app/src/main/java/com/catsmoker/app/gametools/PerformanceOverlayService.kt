...
class PerformanceOverlayService : Service() {
    ...
    private fun initOverlay() {
        ...
        overlayView = inflater.inflate(com.catsmoker.app.R.layout.overlay_performance, root, false) as ViewGroup?

        overlayView?.let {
            powerText = it.findViewById(com.catsmoker.app.R.id.powerNumber)
            cpuText = it.findViewById(com.catsmoker.app.R.id.cpuNumber)
            memText = it.findViewById(com.catsmoker.app.R.id.memoryNumber)
            fpsText = it.findViewById(com.catsmoker.app.R.id.fpsNumber)
            tempText = it.findViewById(com.catsmoker.app.R.id.tempNumber)
        }
    ...
    private fun collectMetrics(): Metrics {
        ...
        // 2. Battery / Power
        val (watts, temp) = readPowerAndTemp()
    ...
    private fun readPowerAndTemp(): Pair<Double, Float> {
        return try {
            ...
            if (batteryIntent != null) {
                ...
                val watts = abs((ua / 1_000_000.0) * (uv / 1_000.0))
                val temp = batteryIntent.getIntExtra(BatteryManager.EXTRA_TEMPERATURE, 0) / 10.0f
                watts to temp
            } else {
                0.0 to 0f
            }
    ...
    private fun calculateFps() {
        var fps = trackedLayer?.let(::getFpsForLayer) ?: -1

        if (fps <= 0) {
            zeroFpsRetry++
            if (zeroFpsRetry > 3 || trackedLayer == null) {
                findActiveLayer()?.let { newLayer ->
