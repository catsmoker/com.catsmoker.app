...
class GameAdapter(
    ...
    override fun onBindViewHolder(holder: GameViewHolder, position: Int) {
        ...
        // Status Text
        val statusText = if (isUnrestricted) {
            ctx.getString(R.string.unrestricted)
        } else {
            ctx.getString(R.string.optimized)
        }
        holder.gamePlayTime.text = info.playTime?.let {
            ctx.getString(R.string.play_time_status_format, it, statusText)
        } ?: statusText
        ...
        holder.launchButton.setOnClickListener {
            val launchIntent = ctx.packageManager.getLaunchIntentForPackage(info.packageName)
