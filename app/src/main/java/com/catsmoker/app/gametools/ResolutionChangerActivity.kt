...
class ResolutionChangerActivity : AppCompatActivity() {
    ...
    private fun showWarningDialog(width: Int, height: Int, dpi: Int) {
        ...
        dialog.safeShow("ResolutionChangerActivity")
    }
    ...
    private fun applyResolution(width: Int, height: Int, dpi: Int) {
        ...
        executeResolutionCommand(
            command = cmd,
            successLog = "Resolution changed: ${width}x${height} @ ${dpi}dpi",
            failureLog = "Failed to apply resolution.",
            successMessageRes = R.string.resolution_apply_success,
            failureMessageRes = R.string.resolution_apply_failed
        )
    }
    ...
    private fun resetResolution() {
        ...
        executeResolutionCommand(
            command = cmd,
            successLog = "Resolution reset to default.",
            failureLog = "Failed to reset resolution.",
            successMessageRes = R.string.resolution_reset_success,
            failureMessageRes = R.string.resolution_reset_failed
        ) {
            binding.widthInput.setText(defaultWidth.toString())
            binding.heightInput.setText(defaultHeight.toString())
            binding.dpiInput.setText(defaultDpi.toString())
    ...
    private suspend fun executeCommandWithSelectedMethod(command: String): Boolean {
        return if (isRootMode() && isRootAvailable) {
            execRoot(command)
        } else {
            execShizuku("sh -c \"$command\"") == 0
        }
    }

    private fun executeResolutionCommand(
        command: String,
        successLog: String,
        failureLog: String,
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            val ok = executeCommandWithSelectedMethod(command)
            withContext(Dispatchers.Main) {
                if (ok) {
                    onSuccessUi?.invoke()
                    log(successLog)
                    showSnackbar(getString(successMessageRes))
                } else {
                    log(failureLog)
                    showSnackbar(getString(failureMessageRes))
                }
            }
        }
    }
    ...
    private suspend fun execShizuku(command: String): Int {
        ...
        if (Shizuku.checkSelfPermission() != PackageManager.PERMISSION_GRANTED) return -1
        if (fileService != null) {
            return try {
        ...
        return suspendCancellableCoroutine { cont ->
            val args =
                Shizuku.UserServiceArgs(ComponentName(packageName, FileService::class.java.name))
                    .daemon(false)
                    .processNameSuffix("resolution_service")
                    .debuggable(BuildConfig.DEBUG)
                    .version(BuildConfig.VERSION_CODE)
